...
class LSTM(object):
    def __init__(self, num_emb, batch_size, emb_dim, hidden_dim,
                 sequence_length, start_token,good_id,pos,
        ...
        with tf.variable_scope('generator'):
            self.g_embeddings = tf.Variable(self.init_matrix_embedding([self.num_emb, self.emb_dim],self.pos))
        ...
        ta_emb_x2 = tensor_array_ops.TensorArray(
        ...
        next_token = tf.cast(tf.reshape(ta_emb_x2.read(0),[self.batch_size]),tf.int32) 
        ...
        def _g_recurrence(i, x_t, h_tm1, gen_o, gen_x):
            ...
            next_token = tf.cast(tf.reshape(tf.argmax(tf.nn.softmax(o_t),1),[self.batch_size]),tf.int32)
            x_tp1 = tf.nn.embedding_lookup(self.g_embeddings,next_token)
            gen_o = gen_o.write(i, tf.reduce_sum(tf.mul(tf.one_hot(next_token, self.num_emb, 1.0, 0.0),tf.nn.softmax(o_t)), 1))  # [batch_size] , prob
        ...
        gen_o_argmax = tensor_array_ops.TensorArray(dtype=tf.float32, size=self.sequence_length,
                                             dynamic_size=False, infer_shape=True)
        gen_x_argmax = tensor_array_ops.TensorArray(dtype=tf.int32, size=self.sequence_length,
                                             dynamic_size=False, infer_shape=True)
        ...
        _, _, _, self.gen_o_argmax, self.gen_x_argmax = tf.while_loop(
            ...
            loop_vars=(tf.constant(0, dtype=tf.int32),
                       tf.nn.embedding_lookup(self.g_embeddings, self.start_token), self.h0, gen_o_argmax, gen_x_argmax))

        self.gen_x_argmax = self.gen_x_argmax.pack()  # seq_length x batch_size
        self.gen_x_argmax = tf.transpose(self.gen_x_argmax, perm=[1, 0])  # batch_size x seq_length
        ###################################################################
        ...
        def _wgantrain_recurrence(i, x_t, h_tm1, g_predictions_wgan):
            ...
            g_predictions_wgan = g_predictions_wgan.write(i, tf.nn.softmax(o_t))  # batch x vocab_size
            next_token = tf.argmax(tf.nn.softmax(o_t),1)
            x_tp1 = tf.nn.embedding_lookup(self.g_embeddings,next_token)
        ...
        _, _, _, self.g_predictions_wgan = tf.while_loop(
            ...
            loop_vars=(tf.constant(0, dtype=tf.int32),
                       ...
                       self.h0, g_predictions_wgan))

        self.g_predictions_wgan = tf.transpose(
            self.g_predictions_wgan.pack(), perm=[1, 0, 2])  # batch_size x seq_length x vocab_size
    ...
    def generate(self, session,x):
        outputs = session.run([self.gen_x],feed_dict={self.x:x})
        return outputs[0]

    def generate_argmax(self, session):
        outputs = session.run([self.gen_x_argmax])
        return outputs[0]
    ...
    def init_matrix(self, shape):
        ...
        print(init)
        return init
    def init_matrix_embedding(self, shape,pos):
        if(pos==1):
