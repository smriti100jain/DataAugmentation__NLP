...
dis_batch_size = 64
dis_num_epochs = 2
dis_alter_epoch = 2#60#100#10#25#50
if(pos == 1):
    positive_file = 'save_pos_pg/real_data.txt'
    negative_file = 'target_generate_pos_pg/generator_sample.txt'
    eval_file = 'target_generate_pos_pg/eval_file.txt'
    test_file = 'target_generate_pos_pg/test_file.txt'
    ...
    inp_ref_file = 'save_pos_pg/input_text_pos.txt'
    test_ref_file = 'save_pos_pg/target_text_pos.txt'
    starting_word_file = 'target_generate_pos_pg/start_word_file.txt'
else:
    positive_file = 'save_neg_pg/real_data.txt'
    negative_file = 'target_generate_neg_pg/generator_sample.txt'
    eval_file = 'target_generate_neg_pg/eval_file.txt'
    test_file = 'target_generate_neg_pg/test_file.txt'
    ...
    inp_ref_file = 'save_neg_pg/input_text_neg.txt'
    test_ref_file = 'save_neg_pg/target_text_neg.txt'
    starting_word_file = 'target_generate_neg_pg/start_word_file.txt'
...
def main():
    ...
    with tf.variable_scope('discriminator'):
        cnn = TextCNN(
            ...
            num_filters=dis_num_filters,pos=pos,
    ...
    if(pos == 1):
        log = open('log_pos_pg/experiment-log.txt', 'w')
    else:
        log = open('log_neg_pg/experiment-log.txt','w')
    #  pre-train generator
    ...
    if(load == 1):
        epoch = 5
        if(pos == 1):
            path = tf.train.get_checkpoint_state('target_generate_pos_pg')
            print(path)
            saver.restore(sess, path.model_checkpoint_path)
            #saver = tf.train.import_meta_graph("target_generate_pos_pg/disc-90.meta")
            #saver.restore(sess,"target_generate_pos_pg/disc-90.ckpt")
        else:
            path = tf.train.get_checkpoint_state('target_generate_pos_pg')
            print(path)
            saver.restore(sess, path.model_checkpoint_path)
    ...
    for epoch in xrange(PRE_EPOCH_NUM):
        ...
        if(pos==1):
            eval_file2 = 'target_generate_pos_pg/eval_file'+'_pretrain_gen_'+str(EPOCHS+epoch)+'.txt'
        else:
            eval_file2 = 'target_generate_neg_pg/eval_file'+'_pretrain_gen_'+str(EPOCHS+epoch)+'.txt'
        ...
        if epoch % 5 == 0:
            ...
            log.write(buffer)
            if(pos==1):
                saver.save(sess,'target_generate_pos_pg/pretrain',global_step=EPOCHS+epoch)
            else:
                saver.save(sess,'target_generate_neg_pg/pretrain',global_step=EPOCHS+epoch)
    ...
    for epoch in range(dis_alter_epoch):
        ...
        if(epoch%30==0):
            if(pos==1):
                saver.save(sess,'target_generate_pos_pg/disc',global_step=EPOCHS+epoch)
            else:
                saver.save(sess,'target_generate_neg_pg/disc',global_step=EPOCHS+epoch)
    ...
    for total_batch in range(TOTAL_BATCH):
        ...
        for it in xrange(start_data_loader.num_batch):
            ...
            if(pos == 1):
                    eval_file2 = 'target_generate_pos_pg/eval_file_reinforce_'+str(EPOCHS+total_batch)+'_'+str(it)+'.txt'
            else:
                    eval_file2 = 'target_generate_neg_pg/eval_file_reinforce_'+str(EPOCHS+total_batch)+'_'+str(it)+'.txt'
            ...
            likelihood_data_loader.create_batches(test_file)
            test_loss = target_loss(sess, generator, likelihood_data_loader)
            print 'reinf-train epoch ', total_batch, 'test_loss ', test_loss,'train_loss',train_loss
            buffer = str(total_batch) + ' test_loss : ' + str(test_loss) + ' train_loss : ' + str(train_loss)+'\n'
